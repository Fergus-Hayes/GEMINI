...
if(t+dt/2d0>=tnext) then    !need to load a new file
  if ( .not. allocated(mlonp)) then    !need to read in the grid data from input file
    ...
    if (myid==0) then    !root process
      !READ IN THE GRID
      write(buf,*) precdir,'/simsize.dat'
      sizefn = trim(adjustl(buf))  ! NOTE: need here trim(adjustl()) for some compilers despite doing it earlier on allocation in io.f90.
      ...
      !NOW READ THE GRID
      write(buf,*) precdir,'/simgrid.dat'
      gridfn = trim(adjustl(buf)) ! NOTE: need here trim(adjustl()) for some compilers despite doing it earlier on allocation in io.f90.
...
if(allocated(mlonp)) then
  ...
  if (allocated(precdatp)) deallocate(precdatp)
end if
...
!! AND TOTAL ENERGY FLUX.  GRID VARIABLES INCLUDE GHOST CELLS
